...
def plan_route_tsp(grid_blocked, blocked_clearance, start_state, obstacles_with_sides):
    ...
    for j, nd in enumerate(feasible):
        ...
        # --- Try clearance grid first, fallback to nominal grid if unreachable ---
        seg = None
        t = float('inf')
        if blocked_clearance is not None:
            seg, t = astar_with_cost(blocked_clearance, start_state, nd["approach"], nd["theta"])
        ...
        if seg:
            ...
            print(f"[WARN] No path (even without clearance) to obstacle #{j+1}")
    ...
    for i, ndi in enumerate(feasible):
        ...
        for j, ndj in enumerate(feasible):
            ...
            #     seg_ij[i][j] = seg
            # --- Try clearance grid first, then fallback to normal grid if needed ---
            seg = None
            t = float('inf')
            if blocked_clearance is not None:
                seg, t = astar_with_cost(blocked_clearance, si, ndj["approach"], ndj["theta"])
...
def task1(json_payload=None):
    ...
    # Normal inflated grid (fits 3x3 robot footprint with no clearance)
    blocked = inflate_blocked(raw_grid, radius=INFLATE_RADIUS)

    # Conservative grid with +1 cell clearance margin
    blocked_clearance = inflate_blocked(raw_grid, radius=INFLATE_RADIUS + 1)
    ...
    full_path, breaks, scans, order = plan_route_tsp(blocked, blocked_clearance, start_state, obstacles_with_sides)
